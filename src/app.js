...
(function () {
  ...
  function legend() {
    return '<div class="legend">' + st('bevestigd') + st('onzeker') + st('te-verifieren') + '<span class="muted">Waarden zonder officiële bron worden niet als feit getoond.</span></div>';
  }
  ...
  function viewVarianten() {
    ...
    return section('tight', '<div class="head"><div class="eyebrow">De vier regelvarianten</div><h2>Zelfde sensor, andere rol</h2><p class="lead">Eerst bepalen wat de sensor in het systeem moet doen. Daarna pas de uitvoering kiezen.</p></div>' + cards + '<p class="small muted" style="margin-top:14px">Beelden: TRILUX klantpresentatie 18-05-2026, p. 9 (B03). De kleur van het beeld zegt niets over de beschikbare kleuren per variant of over de kofferuitvoering.</p>') +
      section('grey', '<div class="head"><div class="eyebrow">In één oogopslag</div><h2>Wat vertel je de klant?</h2></div>' + cmp +
        ...
        '<div class="note"><b>Van standalone naar systeem.</b> De DALI-2 Input Device bestuurt de armaturen niet zelf zoals de Broadcast-sensor. Hij levert aanwezigheid en licht aan het systeem; LiveLink bepaalt groepen, scènes en koppelingen. Bron: trilux.com (A08), TRILUX-klantpresentatie IntuSens (B03, p. 12) en LiveLink-documentatie.</div>');
  }
  ...
  function viewProduct(id) {
    ...
    var art = '<div class="tbl-wrap"><table><thead><tr><th>Variant</th><th>Volledige omschrijving</th><th>TOC</th><th>TK</th><th>Kleur</th><th>Optiek</th><th>Interface</th></tr></thead><tbody><tr>' +
      ...
      '<p class="small muted" style="margin-top:10px">TOC en TK zijn verschillende nummers en worden nooit gelijkgesteld. </p>' + (p.label_foto ? '<div class="label-proof"><img src="' + esc(p.label_foto) + '" alt="Typeplaatje ' + esc(p.kort) + '"><div><b>Typeplaatje</b><br><span class="small muted">Bewijsfoto van het typeplaatje op deze sensor (25-09-2026). Typecode, TOC en TK hierboven zijn hiervan afgelezen.</span></div></div>' : '<p class="small muted">Geen typeplaatje gefotografeerd; artikelgegevens daarom nog niet bevestigd.</p>');
  ...
  function viewStatus() {
    ...
    return section('tight', '<div class="head"><div class="eyebrow">Bronstatus</div><h2>Wat is bevestigd, wat niet?</h2><p class="lead">Deze app toont geen technische gegevens als feit zonder officiële bron. Waarden die nog niet bevestigd zijn, staan als &quot;nog te verifiëren&quot;.</p></div>' +
      ...
      section('grey', '<div class="head"><div class="eyebrow">Artikelen in de koffer</div><h2>TOC en TK per onderdeel</h2></div><div class="tbl-wrap"><table><thead><tr><th>ID</th><th>Onderdeel</th><th>Typecode</th><th>Status</th><th>TOC</th><th>Status</th><th>TK</th></tr></thead><tbody>' + rows + '</tbody></table></div>' + legend() +
        '<h3 style="margin:32px 0 12px">Bronnen</h3><ul class="src-list" style="font-size:14px"><li><b>A08</b> — trilux.com IntuSens-productpagina en productcategorie.</li><li><b>A12</b> — trilux.com lichtpraktijk DALI (algemene DALI-grenzen).</li><li><b>B03</b> — TRILUX IntuSens Customer Presentation 18-05-2026 (via de interne bronaudit).</li><li><b>P01</b> — foto\'s van de demokoffer en de typeplaatjes (25-09-2026).</li><li><b>opgave</b> — projectbrief van 25-09-2026 (typecodes, TOC\'s, kofferinhoud) waar geen typeplaatje beschikbaar is.</li></ul>');
  }
